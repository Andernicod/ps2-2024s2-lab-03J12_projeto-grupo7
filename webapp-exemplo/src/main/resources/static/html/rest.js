        ...
        imoveis.forEach(imovel => {
            ...
            li.innerHTML = `
                ...
                <div id="botoes-ED">
                    ...
                    ` : `
                        <button class="quero-esse-btn" data-email="${imovel.usuario.email}">Quero esse!</button>
                    `}
                </div>
            `;
            imoveisList.appendChild(li);
            const botaoQueroEsse = li.querySelector('.quero-esse-btn');
            if (botaoQueroEsse) {
                botaoQueroEsse.addEventListener('click', () => {
                    const email = botaoQueroEsse.getAttribute('data-email');
                    abrirPopUpQueroEsse(email);
                });
            }
        });
    }
...
function abrirPopUpQueroEsse(email) {
    const popUp = document.createElement('div');
    popUp.classList.add('popup-email');
    popUp.innerHTML = `
        <div class="popup-content">
            <p>Entre em contato com o e-mail: <strong>${email}</strong> <br>para o imóvel.</p>
            <button class="copy-btn">Copiar E-mail</button>
            <button class="close-btn">&times;</button>
        </div>
    `;
    document.body.appendChild(popUp);
    const fecharBtn = popUp.querySelector('.close-btn');
    fecharBtn.addEventListener('click', () => {
        popUp.remove();
    });
    const copiarBtn = popUp.querySelector('.copy-btn');
    copiarBtn.addEventListener('click', () => {
        copiarEmailParaAreaDeTransferencia(email);
    });
...
function copiarEmailParaAreaDeTransferencia(email) {
    setTimeout(() => {
        navigator.clipboard.writeText(email)
            .then(() => {
                mostrarMensagemDeSucesso('E-mail copiado para a área de transferência!');
            })
            .catch(err => {
                mostrarMensagemDeSucesso('Erro ao copiar o e-mail: ' + err);
            });
    }, 100);
}
        ...
        if (imovel) {
            ...
            formEditarImovel.onsubmit = async (event) => {
                ...
                if (responseUpdate.ok) {
                    ...
                    exibirMensagemDeErro(errorText);
                }
            };
        }
    } catch (error) {
    ...
    formRegistro.onsubmit = async (event) => {
        ...
        try {
            const response = await fetch('/api/usuarios', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ nome, sobrenome, email, username, password }),
            });
            const message = await response.text();
